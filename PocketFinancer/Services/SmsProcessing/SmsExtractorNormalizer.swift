...
nonisolated enum SmsExtractorNormalizer {
    ...
    static func minorUnits(decimal: String, currency: String) throws -> Int64 {
        guard decimal.range(of: #"^(0|[1-9][0-9]*)(\.[0-9]+)?$"#, options: .regularExpression) != nil else { throw SmsExtractorValidationError.amountInvalid }
        guard let parsed = CurrencyProfileRegistry.parse(
    ...
    static func minorUnits(evidenceText: String, currency: String) throws -> Int64 {
        ...
        let expression = #"(?<![\w,])(?:[0-9]{1,3}(?:,[0-9]{2})+,[0-9]{3}|[0-9]{1,3}(?:,[0-9]{3})+|[0-9]+)(?:\.[0-9]+)?(?![\w,])"#
        ...
        return try minorUnits(decimal: String(normalized[valueRange]).replacingOccurrences(of: ",", with: ""), currency: currency)
    }
    ...
    static func direction(_ value: String) -> String? {
        ...
        if debit.contains(where: { normalized.range(of: #"\b"# + NSRegularExpression.escapedPattern(for: $0) + #"\b"#, options: .regularExpression) != nil }) {
            return "debit"
        }
        if credit.contains(where: { normalized.range(of: #"\b"# + NSRegularExpression.escapedPattern(for: $0) + #"\b"#, options: .regularExpression) != nil }) {
            return "credit"
        }
        return nil
    }
